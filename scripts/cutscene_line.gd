...
enum Fade {
	...
	TO_BLACK,     ## 黒へ落とす（暗転）
	FROM_BLACK,   ## 黒から戻す（明転）
	TO_WHITE,     ## 白へ飛ばす（ホワイトアウト）
	FROM_WHITE,   ## 白から戻す
	FLASH_WHITE,  ## 白くぱっと光ってすぐ戻る
	FLASH_BLACK,  ## 黒くぱっと沈んですぐ戻る
}
...
# ─────────────────────────────── 幕（暗転・明転・フラッシュ）
@export_group("Fade")
## 画面全体をおおう幕の動かし方。本文・立ち絵・背景ごと隠れる。
##
## 場面を切り替えるときは、前のコマで TO_BLACK、次のコマで背景を差し替えつつ
## FROM_BLACK にする。幕は下りたままコマをまたぐので、間に何コマ挟んでもよい。
##
## 上げ下げ (TO_*/FROM_*) は幕が動ききってから本文が出る。
## フラッシュは待たないので、本文と同時に光る
@export var fade := Fade.NONE
## 幕の動きにかける秒数。0 なら Cutscene 側の既定値 (veil_time) を使う。
## フラッシュではこの秒数のうち最初の少しで光り、残りで戻る
@export_range(0.0, 3.0, 0.05) var fade_sec := 0.0
