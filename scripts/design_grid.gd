...
##
## これは実行中だけのもの。F4 で切り替える。
##
## 編集中は addons/design_grid のプラグインが同じものを 2Dビューに重ねる。
## こちらを @tool にすると、Main.tscn を開いたときだけ二重に描かれて、
## しかもプラグインのボタンでは消せない方が残ってしまう。
## 「実行中はノード、編集中はプラグイン」と持ち場を分けている。
##
